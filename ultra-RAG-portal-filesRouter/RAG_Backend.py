...
# 1. Embedding API
EMBEDDING_API_URL = "https://WWW.BGESICONFLOW.COM:18080/v1/embeddings" 
EMBEDDING_MODEL_NAME = "bge-m3"

# 2. Rerank API
RERANK_API_URL = "https://WWW.BGESICONFLOW.COM:18080/v1/rerank" 
RERANK_MODEL_NAME = "bge-reranker-v2-m3"

# 3. DeepSeek/LLM API
DEEPSEEK_API_URL = "https://WWW.DEEPSEEKVE3.COM:18080/v1/chat/completions"
DEEPSEEK_V3_MODEL_NAME = "DeepSeek-V3"

# 仅用于日志显示的名称
LOCAL_MODEL_NAME_LOG = "BAAI/bge-small-zh-v1.5 (or Local Cache)"
...
class RecallWorker(QThread):
    ...
    def __init__(self, query_text, db_path, json_path, search_mode="smart", summary_model="DeepSeek-R1", 
                 ...
                 # 新增参数
                 use_faiss=True, 
                 enable_local_filter=True,
                 local_filter_instance=None,
                 limit_vector=10,
                 limit_json=10):
        super().__init__()
        ...
        self.airline_names = airline_names if airline_names else [] 
        self.use_faiss = use_faiss     
        
        # 本地小模型相关参数
        self.enable_local_filter = enable_local_filter
        self.local_filter_instance = local_filter_instance
        self.limit_vector = limit_vector # 对应 UI 下拉框 5-20
        self.limit_json = limit_json     # 对应 UI 下拉框 5-20
    ...
    def run(self):
        try:
            ...
            if query_vec_list and os.path.exists(self.db_path):
                ...
                # 计算召回缓冲池大小: 如果开启小模型过滤，先召回更多(如50)进行过滤；否则直接召回 limit_vector
                initial_recall_k = 50 if self.enable_local_filter else self.limit_vector
                ...
                if use_faiss_now:
                    try:
                        ...
                        if embeddings:
                            ...
                            self.log(f"⚡ [FAISS] 检索完成，候选数量: {len(raw_candidates)}")
                ...
                top_candidates_raw = raw_candidates[:initial_recall_k] 
                ...
                # ================= 核心修改：本地小模型过滤 + 有效性验证逻辑 =================
                final_vector_input = []
                target_k = self.limit_vector 

                if self.enable_local_filter and self.local_filter_instance and self.local_filter_instance.is_loaded:
                    self.log(f"🧠 [Local Filter] 正在使用本地模型 ({LOCAL_MODEL_NAME_LOG}) 对 {len(filled_candidates)} 条候选进行精排...")
                    
                    # 1. 记录原始 Top K
                    original_top_ids = set(item['id'] for item in filled_candidates[:target_k])
                    
                    # 2. 计算本地语义分
                    candidate_texts = [c['content'] for c in filled_candidates]
                    semantic_scores = self.local_filter_instance.compute_similarity(self.search_query, candidate_texts)
                    
                    # 3. 将分数赋回对象
                    for i, c in enumerate(filled_candidates):
                        c['local_score'] = semantic_scores[i]
                    
                    # 4. 按本地语义分数排序
                    filled_candidates.sort(key=lambda x: x['local_score'], reverse=True)
                    
                    # 5. 截取 Top K
                    final_vector_input = filled_candidates[:target_k]
                    
                    # 6. 计算捞回数据
                    new_top_ids = set(item['id'] for item in final_vector_input)
                    rescued_ids = new_top_ids - original_top_ids
                    rescue_count = len(rescued_ids)
                    
                    self.log(f"📊 [效用检测] 本地小模型 '捞回' 数据量: {rescue_count}/{target_k}")
                    
                    if rescue_count > 0:
                        self.log(f"   ✅ 有效！小模型将 {rescue_count} 条原本被向量检索排在后面的内容提拔到了 Top {target_k}。")
                        for item in final_vector_input:
                            if item['id'] in rescued_ids:
                                self.log(f"      -> 捞回 ID: {item['id'][:6]}... | Local Score: {item['local_score']:.4f}")
                    else:
                        self.log(f"   ⚖️ 持平。小模型排序与原始向量排序在 Top {target_k} 高度一致。")

                    self.log(f"✅ [Local Filter] 筛选完成，保留 Top {len(final_vector_input)}")
                
                else:
                    if self.enable_local_filter:
                        self.log("⚠️ 本地模型未加载，跳过过滤步骤，直接使用原始向量排序")
                    # 直接截取 limit_vector
                    final_vector_input = filled_candidates[:self.limit_vector]
                
                # Rerank
                rerank_input_texts = [f"Section Path: {c['path']}\nContent: {c['content']}" for c in final_vector_input]
                
                if not self._is_interrupted and final_vector_input:
                    rerank_scores = self.rerank_with_bge(self.search_query, rerank_input_texts)
                    if rerank_scores:
                        for idx, candidate in enumerate(final_vector_input):
                            candidate['rerank_score'] = rerank_scores[idx]
                        
                        final_vector_input.sort(key=lambda x: x.get('rerank_score', 0), reverse=True)
                        self.log(f"✅ Vector 通道准备就绪: {len(final_vector_input)} 条 (已 Rerank)")
                        vector_candidates = final_vector_input
            ...
            if self._json_worker:
                ...
                raw_json = self.json_search_results
                # 应用 limit_json 限制
                if len(raw_json) > self.limit_json:
                    json_results = raw_json[:self.limit_json]
                    self.log(f"✂️ [Limit] JSON 结果限制为前 {self.limit_json} 条 (原 {len(raw_json)} 条)")
                else:
                    json_results = raw_json
            ...
            # 最终展示数量稍微放宽一点，或者也做个限制，这里保持原来的 12
